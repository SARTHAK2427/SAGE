...
class ModelManager:
    ...
    def get_lifecycle_socket(
        ...
        """Full rich Component S socket for model lifecycle events."""
        from core.sockets import build_model_manager_socket, build_error_payload
        ...
        err_payload = build_error_payload("MODEL_LIFECYCLE_ERROR", "ModelLifecycleError", error) if error else None

        return build_model_manager_socket(
            action=action,
            model_key=model_key,
            ...
            mmproj_path=str(model_cfg.get("mmproj")) if model_cfg.get("mmproj") else None,
            context_size=model_cfg.get("context", 8192),
            gpu_layers=model_cfg.get("ngl", 999),
            log_file=str(config.TEMP_DIR / "logs" / f"{model_key}.log"),
