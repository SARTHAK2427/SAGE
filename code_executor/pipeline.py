...
class PipelineResult:
    ...
    # Attempt accounting
    attempts: int = 1                           # 1 = first run only; 2+ = repairs used
    ...
    def to_socket(self, task: str = "") -> dict:
        """Full rich Component R socket."""
        from core.sockets import build_repair_loop_socket
        return build_repair_loop_socket(
            status=self.status,
            ...
            task=task,
            fix_history=[f.to_socket() if hasattr(f, "to_socket") else f for f in self.fix_history],
            infra_error=self.infra_error,
            extraction_had_fence=self.extraction_had_fence,
