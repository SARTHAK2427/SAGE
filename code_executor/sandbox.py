...
class SandboxResult:
    ...
    def to_socket(
        ...
        """Full rich Component P socket."""
        from core.sockets import build_sandbox_execution_socket, build_error_payload
        err_payload = None
        if self.error:
            err_payload = build_error_payload(
                code="DOCKER_INFRA_ERROR",
                type_name="DockerInfraError",
                message=self.error,
            )
        return build_sandbox_execution_socket(
            stdout=self.stdout,
            stderr=self.stderr,
...
class DockerSandbox:
    ...
    def pull_image_if_missing(self) -> None:
        ...
        except (getattr(docker.errors, "ImageNotFound", Exception) if docker else Exception):
    ...
    def _client(self) -> Any:
        ...
        try:
            ...
            client.ping()
            return client
        except (getattr(docker.errors, "DockerException", Exception), Exception) as exc:
            raise RuntimeError(
